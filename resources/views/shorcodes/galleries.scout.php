...
@foreach(GalleryModel::all() as $gallery)
    @include($tmp), [
        ...
        'thumbnail' => $gallery->thumbnailUrl(empty(Meta::get($gallery->id(), 'loopType')) ? 'partials.unique.unique1' : Meta::get($gallery->id(), 'loopType')),
        'date' => $gallery->date(),
        'year' => $gallery->date('Y'),
        'month' => $gallery->date('m'),
        'day' => $gallery->date('d'),
        'description' => Meta::get($gallery->id(), 'description'),
    ])
@endforeach
</div>
