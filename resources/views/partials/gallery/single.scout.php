...
<div class="no-padding no-margin full-width singlepost-header">
    ...
    <header class="animated fadeInUpBig center text-center gallery-caption">
        ...
        <div class="">
            <!--  <a href="{{ Loop::link() }}" rel="bookmark" style="color:inherit"> -->
            <a href="{{ get_day_link(Loop::date('Y'), Loop::date('n') ,Loop::date('j')) }}">
                <time class="" datetime="{{ Loop::date() }}">
                    {{ Loop::date() }}
                </time>
            </a>
            <span>
    ...
    <div class="container gal-wrapper">
        ...
        <div class="my-gallery gal" itemscope itemtype="http://schema.org/ImageGallery">
            @unless(empty(Meta::get(Loop::id(), 'images') ))
                @foreach(Meta::get(Loop::id(), 'images') as $image)
                    <?php
        ...
        @unless(empty(Loop::terms('gallery-tag') ))
            <div class="row">
                <div class="right">
                    @foreach(Loop::terms('gallery-tag') as $tag)
                        <div class="chip">
